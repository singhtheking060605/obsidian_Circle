...
import { runPlagiarismCheck } from '../utils/aiPlagiarismChecker.js';
...
export const runPlagiarismCheckController = catchAsyncError(async (req, res, next) => {
    const { taskId } = req.params;

    const task = await Task.findById(taskId);
    if (!task || !task.submission) {
        return next(new ErrorHandler("Task or submission content not found for plagiarism check.", 404));
    }
    
    // Pass submission content and Task ID (for excluding itself)
    const report = await runPlagiarismCheck(task.submission, taskId); 

    // Update the task document with the report status
    task.plagiarismReport = {
        status: report.status === 'flagged' ? 'flagged' : 'safe',
        maxSimilarity: report.maxSimilarity || 0,
        matchedSubmissions: report.matchedSubmissions || [],
    ...
    if (report.status === 'flagged') {
        res.status(200).json({
            success: true,
            message: 'Plagiarism check completed. Submission flagged.',
            report
        });
    } else {
        res.status(200).json({
            success: true,
            message: 'Plagiarism check completed. Submission appears safe.',
            report
        });
    }
});