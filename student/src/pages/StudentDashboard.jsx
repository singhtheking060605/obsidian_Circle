...
import PersonalDashboard from '../components/PersonalDashboard'; // Import the new component
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'personal-dashboard':
        return <PersonalDashboard />; // Use the component here
      case 'team':
        return <TeamManagement />;
      case 'all-tasks':
        return (
          <div className="space-y-6">
            <h3 className="text-2xl font-bold text-white mb-4 border-l-4 border-red-600 pl-4">Available Missions</h3>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {[1, 2, 3, 4, 5, 6].map((task) => (
                <div key={task} className="p-6 bg-gray-900/40 border border-red-900/20 rounded-lg hover:border-red-500/50 transition-all duration-300 hover:shadow-[0_0_15px_rgba(220,38,38,0.1)] group flex flex-col h-full">
      ...
      case 'assigned':
        return <div className="text-xl text-red-400 font-medium">No active missions assigned. Stay alert.</div>;
      case 'assignments':
        return <div className="text-xl text-red-400 font-medium">Mission history log is empty.</div>;
      case 'alumni-connect':
        return (
          <div className="space-y-6">
            <h3 className="text-2xl font-bold text-white mb-4 border-l-4 border-red-600 pl-4">Alumni Network</h3>
            <div className="grid gap-4 md:grid-cols-2">
               {[1, 2, 3, 4].map((i) => (
                 <div key={i} className="flex items-center gap-4 p-4 bg-black/60 border border-red-900/30 rounded-lg hover:border-red-500 transition-all">
                    <div className="w-12 h-12 rounded-full bg-gray-800 overflow-hidden">
                       <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=Alumni${i}`} alt="Alumni" />
                    </div>
                    <div>
                       <h4 className="font-bold text-white">Senior Agent {i}</h4>
                       <p className="text-xs text-gray-400">Software Engineer at Google</p>
                       <button className="text-xs text-red-500 hover:text-red-400 mt-1">Connect &rarr;</button>
                    </div>
                 </div>
               ))}
            </div>
      ...
      <div className="flex pt-16 relative">
        ...
        >
          ...
          <div className="w-full max-w-[95%] 2xl:max-w-[1800px] mx-auto relative z-10 transition-all duration-300 ease-in-out">
            ...
            <div className={`rounded-xl transition-all duration-300 ${activeTab !== 'team' ? 'bg-black/40 border border-red-900/20 p-6 md:p-8 min-h-[400px] backdrop-blur-sm shadow-2xl shadow-red-900/5' : ''}`}>
              {renderContent()}
            </div>
          </div>
        </main>
