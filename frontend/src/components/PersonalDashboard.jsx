...
// You will need to install react-icons for the FaEdit icon: npm install react-icons
import { FaEdit } from "react-icons/fa";

// Accept setActiveTab as a prop from StudentDashboard
const PersonalDashboard = ({ setActiveTab }) => {
  const { user } = useAuth();
  ...
  const recentActivity = [
    ...
    {
      id: 2,
      text: "Joined The Obsidian Circle.",
      time: "2 days ago",
      type: "success",
    },
  ];
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-end justify-between border-b border-red-900/30 pb-4">
        ...
        <div className="hidden md:block text-right">
          <div className="text-xs text-red-500/60 font-mono mb-1">
            CLEARANCE LEVEL 1
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="bg-black/60 border border-red-900/30 p-6 rounded-xl glow-box hover:border-red-500/50 transition-all group"
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-mono text-gray-500 group-hover:text-red-400">
                UPDATED
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Column: Profile & Status (The card to modify) */}
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-gray-900/40 border border-red-900/30 p-6 rounded-xl text-center relative overflow-hidden">
            ...
            {/* THE NEW UPDATE PROFILE BUTTON */}
            <button
              onClick={() => setActiveTab("update-profile")}
              className="absolute top-3 right-3 p-2 text-xs font-semibold text-red-400 bg-red-900/30 rounded-lg hover:bg-red-600 hover:text-white transition-colors flex items-center gap-1 z-20"
              title="Update Profile"
            >
              <FaEdit className="inline-block w-3 h-3" />
          ...
          {/* Badges Section (omitted for brevity) */}
          <div className="bg-black/40 border border-gray-800 p-6 rounded-xl">
            ...
            <div className="flex gap-2">
              <div
                className="w-10 h-10 bg-red-900/20 border border-red-500/30 rounded-full flex items-center justify-center text-red-500"
                title="Early Adopter"
              >
                ★
              </div>
        ...
        {/* Right Column: Activity Feed (omitted for brevity) */}
        <div className="lg:col-span-2 bg-black/60 border border-red-900/30 rounded-xl p-6 glow-box">
          ...
          <div className="space-y-0 relative">
            ...
            {recentActivity.map((activity) => (
              ...
              >
                <div
                  className={`w-5 h-5 rounded-full border-2 flex-shrink-0 z-10 bg-black ${
